...
# Global variables
image_path="../data/Taxi/images/"
input_path="../data/Taxi/"
...
def objective(allocation, visualize=False):
    ...
    insider_simulator=InsiderSimulator(name_token="Req_",inputAverageArrivalTime=inputAverageArrivalTime,SIMULATION_DURATION=1*60*60,name="Taxi")
...
if __name__ == '__main__':
    ...
    def callback_temptive(intermediate_result):
        print("callback_temptive:" , len(intermediate_result.population_energies))
    ...
    for i, maxiter in enumerate( maxiterList):
        ...
        #objective([0]*len(bounds),True)


        # perform the differential evolution search
        result = differential_evolution( objective, bounds, integrality=(True)*len(bounds), maxiter=maxiter, popsize=1, workers=1, updating='deferred',callback=callback_temptive )
