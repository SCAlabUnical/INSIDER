...
requestUsefulDuration = dict() # per ogni richiesta memorizzo il tempo totale di esecuzione sul nodi, senza i tempi di attesa
rhoCount = dict()
...
def getServiceTime(nameTask, nameToken, tasks):
   ...
   if nameToken in requestUsefulDuration:
       # se la richiesta già esiste aggiungo la duration corrente al totale
        requestUsefulDuration[nameToken] = requestUsefulDuration[nameToken] + duration
   else:
       # alrimenti creo la richiesta e gli setto la duration corrente
       requestUsefulDuration[nameToken] = duration
...
def Tc0_t(c):
    ...
    return [SimToken(name_token+str(curr_token), delay=inputAverageArrivalTime),
            SimToken(c, delay=inputAverageArrivalTime)]
...
class InsiderReporter(Reporter):
    ...
    def callback(self, timed_binding):
        ...
        elif event.get_id() == "Collector": #"Actuator":
            c_id = binding[0][1].value
            #print(c_id)
        ...
        if event.get_id() != "Collector": # "Camera_acquisition":
            waiting=self.wait_time.get(binding[0][1].value,0)
            waiting=waiting+time-binding[0][1].time
            self.wait_time[binding[0][1].value]=waiting
            #print("waiting:", waiting, "-", event.get_id())
...
layoutWF = "./Layout/BeltWasteRecycling.txt"
v = Visualisation(workflow, layoutWF)
v.show("BeltWasteRecycling")
...
print(requestUsefulDuration)
#print("wait_time:",reporter.wait_time)
...
for key in reporter.wait_time.keys():
    if key.startswith(name_token):
        ...
        try:
            ...
            y_wait_time.append( (reporter.complete_times[key]-reporter.arrival_times[key]) - requestUsefulDuration[key] )

        except:
            print("except:",key)


print("y_wait_time:",y_wait_time)


# Arrotondamento delle y per risolvere problema del bug di matplot
for i in range(len(y_complete_times)):
    y_complete_times[i]=round(y_complete_times[i],6)
    y_wait_time[i]=round(y_wait_time[i],6)
#print(x_complete_times)
#print(y_complete_times)

print("rhoCount:", rhoCount)
rhoCount = collections.OrderedDict(sorted(rhoCount.items()))
x_rho = []
y_rho = []
for key in rhoCount:
    x_rho.append(key)
    y_rho.append(round(rhoCount[key]/SIMULATION_DURATION,6))
#print("rhoCount:", rhoCount)

print("resourceUtilizationTime:", resourceUtilizationTime)
#print("resource total utilization time: ",[(k,x/SIMULATION_DURATION) for k,x in resourceUtilizationTime.items()])
...
for key in resourceUtilizationTime:
    ...
    y_ResourceUtilization.append(round(resourceUtilizationTime[key]/SIMULATION_DURATION,6))


# etichette da inserire su asse x dei grafici
xticks = []
for i in range(len(x_complete_times)):
    if i % 20 == 0:
        xticks.append(x_complete_times[i])
...
plt.savefig("./images/BeltWasteRecycling_wait_time.png")
plt.close()
...
plt.savefig("./images/BeltWasteRecycling_cycle_time.png")
plt.close()
...
plt.savefig("./images/BeltWasteRecycling_system_load.png")
plt.close()
...
plt.ylabel("RHO")
plt.savefig("./images/BeltWasteRecycling_RHO.png")
plt.close()
...
plt.yticks(np. arange(0, 1.1, step=0.2))
plt.ylabel("RHO")
plt.savefig("./images/BeltWasteResourceUtilization.png")
plt.close()

print("mean_cycle_time:",reporter.mean_cycle_time())
print("mean_system_load:", reporter.mean_system_load())
print("request total work time: ",requestUsefulDuration)
#########################################