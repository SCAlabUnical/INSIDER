...
### Modifica start
COLOR_COMPUTE=(0,255,0)
COLOR_NETWORK=(0,255,255)
COLOR_STORAGE=(255,255,0)
### Modifica end
...
class PlaceViz(Node):
    ...
    def draw(self, screen):
        ### Modifica start
        # print("ciao")
        insidertag = self._model_node.insider_tag
        #print(insidertag)
        color = TUE_LIGHTBLUE
        if insidertag['type'] == "COMPUTE":
            color = COLOR_COMPUTE
        if insidertag['type'] == "NETWORK":
            color = COLOR_NETWORK
        if insidertag['type'] == "STORAGE":
            color = COLOR_STORAGE

        pygame.draw.circle(screen, color, (self._pos[0], self._pos[1]), self._half_height)
        ### Modifica end
        ...
        if "labelName" in insidertag and insidertag["labelName"] == "no":
            None
        else:
            # draw label
...
class TransitionViz(Node):
    ...
    def draw(self, screen):
        ...
        pygame.draw.rect(screen, color,
                         pygame.Rect(self._pos[0] - self._half_width, self._pos[1] - self._half_height, self._width,
                                     self._height))
        #### Modifica end
        pygame.draw.rect(screen, TUE_BLUE,
                         pygame.Rect(self._pos[0] - self._half_width, self._pos[1] - self._half_height, self._width,
                                     self._height), LINE_WIDTH)
        font = pygame.font.SysFont('Calibri', TEXT_SIZE)
        ...
        ### Modifica start
        if "flow" in insidertag:
            myfont = pygame.font.SysFont("monospace", 25)
            # render text
            label = myfont.render(insidertag["flow"], 1, (0,0,255))
            screen.blit(label, (text_x_pos-20, text_y_pos+20))

            if str(insidertag["flow"]).lower().__contains__("and"):
                pygame.draw.rect(screen, TUE_RED,
                                 pygame.Rect(self._pos[0] + 0.6*self._half_width, self._pos[1] + 0.6*self._half_height, self._width/3,
                                             self._height/3))
            if str(insidertag["flow"]).lower().__contains__("or"):
                pygame.draw.circle(screen, TUE_RED, (self._pos[0] - self._half_width, self._pos[1]+ self._half_height), self._half_height/2.5)
...
class Visualisation:
        ...
        with open(filename, "w") as f:
            ...
            for node in self._nodes.values():
                if "," in node.get_id() or "\n" in node.get_id():
                    raise Exception(
                        "Node " + node.get_id() + ": Saving the layout cannot work if the node id contains a comma or hard return.")
    ...
    def __get_node_at(self, pos):
        for node in self._nodes.values():
            if node.get_pos()[0] - node._width / 2 <= pos[0] <= node.get_pos()[0] + node._width / 2 and node.get_pos()[
                1] - node._height / 2 <= pos[1] <= node.get_pos()[1] + node._height / 2:
                return node
        return None
    ...
    def show(self,filename):
        ...
        while self.__running:
            ...
            pygame.image.save(pygame.display.get_surface(),"./images/"+filename+"_PetriNet.png")
